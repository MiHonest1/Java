        ...
        CalcButton.addActionListener(new ActionListener() { // ActionListener хранит в себе метод, который активируется при нажатии кнопки
            ...
            public void actionPerformed(ActionEvent e) { // actionPerformed - метод, который активируется при нажатии кнопки
                ...
                for (int k = 0; k < MyModel.getRowCount(); k++) {
                    Vector data = MyModel.getDataVector().get(k); // в data записываем значения выбранной строки (значения могут меняться)
                    ...
                    MyModel.setValueAt(area, k, 3); // установка значения интеграла в строку с индексом в столбце №3
                    info.get(k).Res = area;//Работа с коллекцией
                }
